...
// function fillBox(grid: [number, number], row: number, col: number) {
//   let num;
//   for (let i = 0; i < 3; i++) {
//     for (let j = 0; j < 3; j++) {
//       do {
...
// function fillDiagonal(grid: [number, number]) {
//   for (let i = 0; i < 9; i += 3) {
...
// function fillRemaining(grid: [number, number], i: number, j: number) {
...
// function removeKDigits(grid: [number, number], k: number) {
//   while (k > 0) {
//     // Pick a random cell
...
// function sudokuGenerator(k: number) {
//   // Initialize an empty 9x9 grid
//   let grid = new Array(9)
//     .fill(0)
//     .map(() => new Array(9).fill(0)) as unknown as [number, number];
